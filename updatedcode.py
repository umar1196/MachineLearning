...
def generate_sub_db(user, pasword, query_db_name, descrip, db_name, gene_id, flag, given_database_id):
    ...
    try:
        ...
        try:
            ...
            if flag == 'yes': #retrive all biodatabase id sequences
                ...
                cursor.execute('select s.seq, b.accession from biosequence s left join bioentry b on s.bioentry_id = b.bioentry_id left join biodatabase bio on bio.biodatabase = b.biodatabase')
                for i,g in cursor:
                    print(i,g)
                record = cursor.fetchall()
                for z in range(len(record)):
